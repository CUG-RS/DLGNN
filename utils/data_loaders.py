...
class SRDataset(torch.utils.data.dataset.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #print(img_lr.shape)   #(120, 120, 3)
        img_lr, img_hr = self.transforms(img_lr, img_hr)
        #print(img_lr.shape)  #torch.Size([3, 10, 10])
        #print("img_lr.type:",type(img_lr))
        '''
        if cfg.CONST.SCALE == 4:
    ...
    def image_read(self, idx):
        #print(self.file_list[idx]['img_name'])
        #print(self.file_list[idx]['img_lr'])
        #img_lr_idx=cv2.imread(self.file_list[idx]['img_lr'])
        #print(img_lr_idx)
        #img_hr_idx = cv2.imread(self.file_list[idx]['img_hr'])
        #print(img_hr_idx)
        #print(self.file_list[idx]['img_hr'])
...
class SRDataLoader:
    ...
    def get_files(self, phase, samples):
        ...
        for sample_idx, sample_name in enumerate(samples):
            ...
            #t=os.path.exists(img_lr_path)
            #print(t)
            if os.path.exists(img_lr_path) and os.path.exists(img_hr_path):
                files.append({
                    'img_name': sample_name,
                    'img_lr'  : img_lr_path+'/'+sample_name,
                    'img_hr'  : img_hr_path+'/'+sample_name
                })
            '''
            if os.path.exists(img_lr_path) and os.path.exists(img_hr_path):
                files.append({
...
DATASET_LOADER_MAPPING = {
    ...
    'Demo': TestDataLoader,
    'remote_val': SRDataLoader,
    'remote_train':SRDataLoader,
    'UCMerced_val': SRDataLoader,
    'UCMerced_train':SRDataLoader,
    'UCMerced_test':SRDataLoader,
    'nwpu_train':SRDataLoader,
    'nwpu_val':SRDataLoader,
    'rsdata_train':SRDataLoader,
    'rsdata_val':SRDataLoader,
    'rsdata_test':SRDataLoader,
    'ucasaod_train':SRDataLoader,
    'ucasaod_val':SRDataLoader,
    'ggp':SRDataLoader,
    'massets_train':SRDataLoader,
    'massets_val':SRDataLoader,
    'massets_test':SRDataLoader,
    'tianjin_train':SRDataLoader,
    'tianjin_val':SRDataLoader,
    'tianjin_test':SRDataLoader
