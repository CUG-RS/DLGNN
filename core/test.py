...
def test(cfg, test_data_loader, net, Best_Epoch):
    ...
    cur_test_name = (dt.now().isoformat()+'_'+cfg.DIR.DATASET_SCALE).replace(":","-")
    ...
    for batch_idx, (img_name, img_lr_s, img_lr, img_hr) in enumerate(test_data_loader):
        ...
        # Get data from data loader
            ...
            '''
            print("img_out_y.shape:",img_out_y.shape)
            print("img_hr_y.shape:", img_hr_y.shape)
            if img_out_y.shape[0] - img_hr_y.shape[0] != 0 or img_out_y.shape[1] - img_hr_y.shape[1] != 0:
                continue
            '''
            # Calculate PSNR
