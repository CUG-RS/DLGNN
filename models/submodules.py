...
class Graph(nn.Module):
    ...
    def __init__(self, scale, k=5, patchsize=3, stride=1, window_size=20, in_channels=256, embedcnn=None):
        ...
        #print("indexer.shape:", indexer.shape)
...
class GCNBlock(nn.Module):
    ...
    def weight_edge(self, knn_hr, diff_patch):
        ...
        #print("knn_lr.shape:",knn_lr.shape)
        #print("knn_hr.shape:", knn_hr.shape)
...
def compute_distances(xe, ye, I, train=True):
    ...
    :param xe: BxNxE tensor of database (son) item embeddings  经过vgg[3]得到的tensor
    :param ye: BxMxE tensor of query (father) item embeddings  经过vgg[3]得到的tensor
    :param I: BxMxO index tensor that selects O potential neighbors in a window for each item in ye  邻居索引值矩阵
    :param train: whether to use tensor comprehensions for inference (forward only)

    :return D: a BxMxO tensor of distances   距离矩阵
    """
    ...
    if train or not cfg.NETWORK.WITH_WINDOW:  # WITH_WINDOW:true   如果是训练集或者没使用window
        # D_full -> b m n
        D = ops.euclidean_distance(ye, xe.permute(0, 2, 1))  # b m e （b,e,n)
        if cfg.NETWORK.WITH_WINDOW:  # true
            # D -> b m o
...
def hard_knn(D, k, I):
    ...
    """
    # print("D:",D)
    score, idx = torch.topk(D, k, dim=2, largest=False, sorted=True)  # 距离和对应的索引值
    # print("score:",score)
    # print("idx:",idx)
    if cfg.NETWORK.WITH_WINDOW:
        idx = I.gather(dim=2, index=idx)  # I是邻居矩阵
...
class GraphConstruct(nn.Module):
    ...
    def graph_k(self, xe, ye, I):
        ...
        # hard knn
        # return: b m k
        score_k, idx_k = hard_knn(D, k, I)  # 返回距离和索引
        ...
        if cfg.NETWORK.WITH_DIFF:  # true
            ...
            diff_patch = ye_e - torch.gather(xe_e, dim=3, index=idx_k_e)  # query 和 neighbors的差值矩阵
        else:
            diff_patch = None

        if cfg.NETWORK.WITH_SCORE:  # false
    ...
    def forward(self, xe, ye):
        r"""
        :param xe: embedding of son features     neighbors
        :param ye: embedding of father features  query
        ...
        H, W = ye.shape[2:]
        # ("xe.type:",type(xe))  #<class 'torch.Tensor'>
        # print("xe.shape:", xe.shape)   #torch.Size([4, 64, 5, 5])
        # print("ye.shape:", ye.shape)    #([4, 64, 10, 10])
        # vutils.save_image(xe[:,2:5,:,:], './z/' + 'xe.jpg', normalize=True)
        xe_patch = ops.im2patch(xe, self.patchsize, self.stride, self.padding)
        # ("xe_patch.type:", type(xe_patch))
        # print("xe_patch.shape:", xe_patch.shape)  #torch.Size([4, 64, 3, 3, 2, 2])
        ...
        _, _, _, _, n1, n2 = xe_patch.shape  ##torch.Size([4, 64, 3, 3, 2, 2])
...
class GraphAggregation(nn.Module):
    ...
    def aggregation(self, yd, idx_k):
        r"""
        :param yd: database items, shape BxNxF     #torch.Size([1, 4, 9216])
        :param idx_k: indexs of top k nearest neighbors    #torch.Size([1, 25, 4])
        ...
        b, n, f = yd.shape
        k = self.k
        # print("idx_k.shape:", idx_k.shape)   #torch.Size([4, 25, 4])
        # yd -> b m f n
        # idx-> b m f k
        yd = yd.permute(0, 2, 1).contiguous()
        # print("yd.shape:", yd.shape)   #torch.Size([4, 9216, 4])
        yd_e = yd.view(b, 1, f, n).expand(b, m, f, n)
        idx_k_e = idx_k.view(b, m, 1, k).expand(b, m, f, k)
        z = torch.gather(yd_e, dim=3, index=idx_k_e)
        # print("z.type:",type(z))    #
        # print("z.shape:", z.shape)   #torch.Size([4, 25, 9216, 4])
        # vutils.save_image(z, './z/'  + 'z.jpg', normalize=True)
        # b m1*m2 c*p1*p2 k
        return z
    ...
    def forward(self, y, yd, idx_k):
        ...
        # Adaptive_instance_normalization 自适应网络
