...
class Model(nn.Module):
    def __init__(self, opt, ckp):
        ...
        self.model = model.drn.make_model(opt).to(self.device)
        #self.dual_models = []
        self.dual_model = DownBlock(opt, 4).to(self.device)   #scale=4（opt, 4）
        '''
        ...
        if not opt.cpu and opt.n_GPUs > 1:
            ...
            self.dual_model = dataparallel(self.dual_model, range(opt.n_GPUs))
        ...
        if not opt.test_only:
            ...
            print(self.dual_model, file=ckp.log_file)
    ...
    def save(self, path, is_best=False):
        ...
        '''
        torch.save(self.get_dual_model().state_dict(),os.path.join(path, 'model', 'dual_model_latest.pt'))
        if is_best:
            torch.save(
                self.get_dual_model().state_dict(),
                os.path.join(path, 'model', 'dual_model_best.pt')
            )



    def load(self, pre_train='.', pre_train_dual='.', cpu=False):#ori:cpu=False
        if cpu:
        ...
        if pre_train != '.':
            ...
            self.get_model().load_state_dict(
                #torch.load(pre_train, **kwargs,),# ori
                # torch.load(pre_train,map_location=torch.device('cpu'),),
                torch.load(pre_train, map_location=torch.device('cuda'), ),
                #torch.load(pre_train,map_location=torch.device('cuda' if torch.cuda.is_available() else 'cpu')),
                strict=False
            )
        #### load dual model ####
        print("pre_train:", pre_train_dual)
        if pre_train_dual != '.':
            print('Loading dual model from {}'.format(pre_train_dual))
            dual_model = torch.load(pre_train_dual,
                            #**kwargs
            ...
            '''
            self.get_dual_model().load_state_dict(dual_model,strict=False)
